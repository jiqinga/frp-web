 ...
 * @LastEditors         : 寂情啊
 * @LastEditTime        : 2025-12-31 17:04:19
...
func (s *ClientService) CreateClient(client *model.Client) error {
	...
	if client.FrpcAdminPort == 0 {
		...
		log.Printf("[客户端创建] 自动设置 Admin API 配置: addr=%s, port=%d, user=%s",
...
func (s *ClientService) DeleteClient(id uint) error {
	...
	if websocket.ClientDaemonHubInstance.IsClientOnline(id) {
		log.Printf("[客户端删除] 客户端 ID=%d 在线，发送停止命令...", id)
		if err := websocket.ClientDaemonHubInstance.SendShutdownCommand(id); err != nil {
			log.Printf("[客户端删除] ⚠️ 发送停止命令失败: %v，继续删除操作", err)
		} else {
			log.Printf("[客户端删除] ✅ 停止命令已发送，等待 daemon 关闭...")
			// 等待短暂时间让 daemon 有机会优雅关闭
			time.Sleep(500 * time.Millisecond)
		}
	} else {
		log.Printf("[客户端删除] 客户端 ID=%d 不在线，跳过发送停止命令", id)
	}
	...
	return database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("client_id = ?", id).Delete(&model.Proxy{}).Error; err != nil {
			log.Printf("[客户端删除] ❌ 删除客户端 ID=%d 的关联代理失败: %v", id, err)
			return err
		}
		log.Printf("[客户端删除] ✅ 已删除客户端 ID=%d 的所有关联代理", id)

		// 再删除客户端记录
		if err := tx.Delete(&model.Client{}, id).Error; err != nil {
			log.Printf("[客户端删除] ❌ 删除客户端 ID=%d 失败: %v", id, err)
			return err
		}
		log.Printf("[客户端删除] ✅ 客户端 ID=%d 删除成功", id)
	...
	for _, client := range clients {
		...
		if client.WsConnected != wsConnected {
			if err := s.clientRepo.UpdateWSStatus(client.ID, wsConnected); err != nil {
				log.Printf("[WS状态同步] ❌ 更新客户端 %s WS状态失败: %v", client.Name, err)
			}
		}
	}
	return nil
...
func (s *ClientService) UpdateHeartbeat(clientID uint) error {
	now := time.Now()
	log.Printf("[心跳上报] 收到客户端 ID=%d 的心跳，时间: %v", clientID, now.Format("2006-01-02 15:04:05"))
	if err := s.clientRepo.UpdateOnlineStatus(clientID, "online", now); err != nil {
		log.Printf("[心跳上报] ❌ 更新失败: %v", err)
		return err
	}
	log.Printf("[心跳上报] ✅ 客户端 ID=%d 状态已更新为 online", clientID)
	return nil
}
...
func (s *ClientService) UpdateOnlineStatusDirectly(clientID uint, status string) error {
	now := time.Now()
	log.Printf("[WS状态更新] 客户端 ID=%d 状态更新为: %s", clientID, status)
	if err := s.clientRepo.UpdateOnlineStatus(clientID, status, now); err != nil {
		log.Printf("[WS状态更新] ❌ 更新失败: %v", err)
		return err
	}
	log.Printf("[WS状态更新] ✅ 客户端 ID=%d 状态已更新为 %s", clientID, status)
	return nil
}
...
func (s *ClientService) ResetAllClientStatus() error {
	log.Println("[客户端状态] 重置所有客户端状态为离线...")
	if err := s.clientRepo.ResetAllClientStatus(); err != nil {
		log.Printf("[客户端状态] ❌ 重置失败: %v", err)
		return err
	}
	log.Println("[客户端状态] ✅ 所有客户端状态已重置为离线")
	return nil
}
...
func (s *ClientService) UpdateConfigSyncStatus(clientID uint, success bool, errorMsg string, rolledBack bool) error {
	...
	syncTime := time.Now()
	log.Printf("[配置同步状态] 客户端 ID=%d 同步结果: success=%v, status=%s, error=%s", clientID, success, status, errorMsg)

	if err := s.clientRepo.UpdateConfigSyncStatus(clientID, status, errorMsg, syncTime); err != nil {
		log.Printf("[配置同步状态] ❌ 更新失败: %v", err)
		return err
	}
	log.Printf("[配置同步状态] ✅ 客户端 ID=%d 状态已更新为 %s", clientID, status)
	return nil
}
...
func (s *ClientService) SetConfigSyncPending(clientID uint) error {
	syncTime := time.Now()
	log.Printf("[配置同步状态] 客户端 ID=%d 设置为 pending", clientID)
